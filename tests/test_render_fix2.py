...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, 'debug_render2.log'), encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('wallmock_debug2')
...
def check_screen_content(img, test_name):
    w, h = img.size
    
    center_x = w // 2
    center_y = h // 2
    
    screen_w = min(w // 2, 600)
    screen_h = min(h // 2, 1000)
    
    screen_x = center_x - screen_w // 2
    screen_y = center_y - screen_h // 2
    ...
    logger.info(f"[{test_name}] 非黑像素数: {non_black_count}/{total_pixels}")
    ...
    samples = []
    for y in range(0, screen_h, max(1, screen_h // 30)):
        for x in range(0, screen_w, max(1, screen_w // 30)):
            pixel = screen.getpixel((x, y))
            r, g, b = pixel[:3]
    ...
    logger.info(f"[{test_name}] 检测到颜色: 红={has_red}, 绿={has_green}, 蓝={has_blue}, 黄={has_yellow}, 白={has_white}")
    
    if has_red or has_green or has_blue or has_yellow or has_white:
        return True
    
    logger.error(f"[{test_name}] 未检测到测试图中的颜色！")
    return False
...
def test_all_templates():
    ...
    for tpl_id in templates:
        ...
        try:
            ...
            passed = check_screen_content(result, tpl_id)
